...
# -------------------
# Select only desired groups
# -------------------
selected_data <- viability %>%
  filter(group %in% c("Healthy", "Tegaderm_50", "New_Mesh_50", "Toxic"))
...
# Rename groups for plotting and add mean in x-axis label
summary_data$group_label <- factor(summary_data$group,
                                   levels = c("Healthy", "New_Mesh_50", "Tegaderm_50", "Toxic"),
                                   labels = c(
                                     paste0("Healthy\nMean: ", round(summary_data$mean_value[summary_data$group=="Healthy"],1), "%"),
                                     paste0("Nylon\nMean: ", round(summary_data$mean_value[summary_data$group=="New_Mesh_50"],1), "%"),
                                     paste0("Tegaderm\nMean: ", round(summary_data$mean_value[summary_data$group=="Tegaderm_50"],1), "%"),
                                     paste0("Toxic\nMean: ", round(summary_data$mean_value[summary_data$group=="Toxic"],1), "%")
                                   ))

# -------------------
# Bar plot with mean in x-axis labels
# -------------------
p_viability <- ggplot(summary_data, aes(x = group_label, y = mean_value)) +
  geom_bar(stat = "identity", fill = "red", width = 0.4) + # narrow bars
  geom_errorbar(aes(ymin = mean_value - sd_value, ymax = mean_value + sd_value),
                width = 0.15, color = "black", size = 0.7) +
  labs(x = "Material Type", y = "Cell Viability (%)") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text.x = element_text(face = "bold", color = "black", size = 12, lineheight = 0.9),
    axis.title = element_text(face = "bold", size = 14),
    plot.title = element_text(face = "bold", size = 16),
    panel.grid.major.x = element_blank(),
    panel.grid.minor = element_blank()
  ) +
  ggtitle("Cell Viability Across Selected Materials")
